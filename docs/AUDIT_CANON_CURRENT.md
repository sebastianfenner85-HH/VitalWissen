...
**Stand:** 07.05.2026 (S3-PIPELINE-SPEC)  
...
### S3 — Studienkompass
**Status:** ✅ Freeze (P7D-03, 19.04.2026). ✅ **S3-K6-SCHEMA-SPEC abgeschlossen (07.05.2026)** — Bindende Schema-Spezifikation Kernobjekt K6: 38 Felder (A–T), API-Quellen (OpenAlex/PubMed/Crossref), Curation-Workflow K.1 (vollständig manuell, KI-TLDR VERBOTEN), Hype-Guardrails, RLS E29-konform. P.0 Pflicht-Spec-Sequenz (4 Pakete vor S3-BUILD-01). ✅ **S3-MVP-SLICE-SPEC + Customer-Value-Patch abgeschlossen (07.05.2026)** — Option C (Krankheiten-only/K1): 5 ICD-Cluster (I10/E11/F32/E03/D50), 20 manuell kuratierte K6-Einträge (4 je Cluster), Modus 1 (Cross-Block [15] auf S5-Detailseiten). C.1 = kuratierter Krankheitsstudien-Slice, kein vollständiger Studienkatalog. K4 Supplements / K5 Medikamente / K3 Laborwerte / K7 Maßnahmen: nicht in C.1. ICD-Codes = interne Objektanker; PubMed/OpenAlex nutzt kuratierte MeSH-nahe Suchterme (Mapping Aufgabe von S3-PIPELINE-SPEC). H0: Coverage-Regel, Findability-Tabelle, Empty-State-Regel, UX-Claim. Originalabstract C.1 NEIN. CSS-Prefix `s3-*`. Keine Build-Freigabe.  
✅ **S3-PIPELINE-SPEC abgeschlossen (07.05.2026)** — Vollständige Pipeline-Spec (Sections A–S): ICD→MeSH-Mapping (5 Cluster, Primary/Secondary/Tertiary), zweistufige Suchstrategie (Quick-Pass + Deep-Pass), Quellen-Prioritätskette (OpenAlex→PubMed→Crossref→EuropePMC→DOI.org; Cochrane/Semantic Scholar ROT), Pipeline-Phasen P0–P8 (vollständig manuell C.1), Curation-Queue-Format, Gates K.1–K.4, API-/Rate-/Secrets-Regeln, K6-Feldmapping J.1–J.5, Coverage-/Findability-/Empty-State-Regeln, 15 Validatoren PASS. Kein Code, kein DB-Write, kein Commit.  
Nächster Schritt: **S3-UX-SPEC** — eigenständiger Chat, explizite Freigabe. (11-Block-Detailseite `/studien/:slug`, Block [15] auf S5, CSS-Prefix `s3-*`, Studientyp-Ampel, Quellenbox Q2-Typ `research`)  
Führend: `01_PROJECT_SOURCES_CURRENT/S3_PIPELINE_SPEC.md` · `01_PROJECT_SOURCES_CURRENT/S3_MVP_SLICE_SPEC.md` · `01_PROJECT_SOURCES_CURRENT/S3_K6_SCHEMA_SPEC.md`
...
*Nächste Pflicht-Aktualisierung: bei S3-UX-SPEC, Q2-BUILD-02c-P2B, S8-BUILD-02d, B4-BUILD-03, UI-REFRESH-04 oder jedem anderen Paket-Abschluss*
